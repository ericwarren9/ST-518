...
## Part C (**Double check with Module 3 Notes**)

What does it mean for the effects of water and seed age to be plausibly additive?

If we are saying the effects to be plausibly additive, we are saying that these effects of water and seed age do not interact and that each effect has an additive effect on the response variable which in this case is y (or the number of seeds that sprout).
...
```{r problem 3 anova}
(anova_table3 <- anova(lm(y ~ (as.factor(a) + as.factor(b) + as.factor(c)) ^2 + as.factor(a):as.factor(b):as.factor(c), shrimp)))
```

From this table we can pull our MS(E) to be `r anova_table3$"Mean Sq"[[8]]`. We will use that value as we do our F-tests on our contrasts. Note $F = \frac{SS(\hat{\theta}) / (a-1)(c-1)}{MS(E)}$ on degrees of freedom (a-1)(c-1), abc(n-1). Now, $(a-1)(c-1) = (2-1)(3-1) = 1*2 = 2$ and $abc(n-1) = 2*2*3(3-1) = 12(2) = 24$. Plugging everything in our F-tests will consist of $F = \frac{SS(\hat{\theta}) / (a-1)(c-1)}{MS(E)} = \frac{SS(\hat{\theta}) / 2}{2903.778}$ on degrees of freedom 2, 24. For each $\hat{\theta_i}$ we are testing, we want the null hypothesis to be that $H_0: \theta_i = 0$ with the alternative hypothesis being that $H_A: \theta_i \neq 0$. Also note, we will be using a significance level of $\alpha = 0.05$ so we are comparing if our F-value we obtain is larger than the F-critical value of F(.05, 2, 24) = `r qf(0.05, 2, 24, lower.tail = F)`.
