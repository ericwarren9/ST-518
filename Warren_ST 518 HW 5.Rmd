...
To find this we need to look at the mean difference of batteries with the different duties with no fixed variable. Thus, we are going to look at the difference of means between battery types 1 and 2 and then compare to types 3 and 4. Thus, $\theta_3 = (1, 1, -1, -1)' \begin{pmatrix} \mu_1 \\ \mu_2 \\ \mu_3 \\ \mu_4 \end{pmatrix} = \mu_1 + \mu_2 - \mu_3 - \mu_4$ so to find the estimate we are going to say that $\hat{\theta_3} = \hat{\mu_1} + \hat{\mu_2} - \hat{\mu_3} - \hat{\mu_4} = \bar{y_1} + \bar{y_2} - \bar{y_3} - \bar{y_4}$. We know $\bar{y_1} =$ `r mean_output$mean[[1]]`, $\bar{y_2} =$ `r mean_output$mean[[2]]`, $\bar{y_3} =$ `r mean_output$mean[[3]]`, and $\bar{y_4} =$ `r mean_output$mean[[4]]`. $\hat{\theta_3} = \bar{y_1} + \bar{y_2} - \bar{y_3} - \bar{y_4} =$ `r mean_output$mean[[1]] + mean_output$mean[[2]] - mean_output$mean[[3]] - mean_output$mean[[4]]`. Thus, the main effect of Duty batteries is `r mean_output$mean[[1]] + mean_output$mean[[2]] - mean_output$mean[[3]] - mean_output$mean[[4]]`.
...
To find this we need to look at the mean difference of batteries with the different brands with no fixed variable. Thus, we are going to look at the difference of means between battery types 1 and 3 and then compare to types 2 and 4. Thus, $\theta_5 = (1, -1, 1, -1)' \begin{pmatrix} \mu_1 \\ \mu_2 \\ \mu_3 \\ \mu_4 \end{pmatrix} = \mu_1 - \mu_2 + \mu_3 - \mu_4$ so to find the estimate we are going to say that $\hat{\theta_5} = \hat{\mu_1} - \hat{\mu_2} + \hat{\mu_3} - \hat{\mu_4} = \bar{y_1} - \bar{y_2} + \bar{y_3} - \bar{y_4}$. We know $\bar{y_1} =$ `r mean_output$mean[[1]]`, $\bar{y_2} =$ `r mean_output$mean[[2]]`, $\bar{y_3} =$ `r mean_output$mean[[3]]`, and $\bar{y_4} =$ `r mean_output$mean[[4]]`. $\hat{\theta_5} = \bar{y_1} - \bar{y_2} + \bar{y_3} - \bar{y_4} =$ `r mean_output$mean[[1]] - mean_output$mean[[2]] + mean_output$mean[[3]] - mean_output$mean[[4]]`. Thus, the main effect of brand batteries is `r mean_output$mean[[1]] - mean_output$mean[[2]] + mean_output$mean[[3]] - mean_output$mean[[4]]`.
