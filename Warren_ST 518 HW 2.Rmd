...
As we can see from our summary chart, we can see that the standard error for our slope is 18.32.
...
Now to find the standard error in the difference between a 2 and 4 bedroom house, we can say that 
$$\hat{SE}(\hat{\mu}(x_0-x_1))$$ 
$$= \sqrt{(\hat{SE}(\hat{\mu}(x_0)))^2 + (\hat{SE}(\hat{\mu}(x_1)))^2}$$
$$= \sqrt{\sqrt{MSE(\frac{1}{n}+\frac{(x_0-\bar{x})^2}{\sum(x_i-\bar{x})^2})}^2 + \sqrt{MSE(\frac{1}{n}+\frac{(x_1-\bar{x})^2}{\sum(x_i-\bar{x})^2})}^2}$$
$$= \sqrt{MSE(\frac{1}{n}+\frac{(x_0-\bar{x})^2}{\sum(x_i-\bar{x})^2}) + MSE(\frac{1}{n}+\frac{(x_1-\bar{x})^2}{\sum(x_i-\bar{x})^2})}$$
$$= \sqrt{MSE(\frac{1}{n}+\frac{(x_0-\bar{x})^2}{\sum(x_i-\bar{x})^2} + \frac{1}{n}+\frac{(x_1-\bar{x})^2}{\sum(x_i-\bar{x})^2})}$$
$$= \sqrt{MSE(\frac{2}{n}+\frac{(x_0-\bar{x})^2}{\sum(x_i-\bar{x})^2}+\frac{(x_1-\bar{x})^2}{\sum(x_i-\bar{x})^2})}$$
In the code chunk below we are solving for the $\hat{SE}(\hat{\mu}(x_0-x_1))$. Please use the following code: `((summary(priceForBedrooms)$sigma) ** 2)` which will provide us with our MSE value.
```{r possible formula for MSE testing}
# Plug in the values of x_0 and x_1
x_0 = 4
x_1 = 2

# Solve the MSE(mu(x_0-x_1))
se_x0_subtract_x1 <- sqrt(
  ((summary(priceForBedrooms)$sigma) ** 2) * (
    (2 / nrow(houses)) + (
      ((x_0 - mean(houses$bedrooms)) ** 2) / (sum((houses$bedrooms - mean(houses$bedrooms)) ** 2))
    ) + (
      ((x_1 - mean(houses$bedrooms)) ** 2) / (sum((houses$bedrooms - mean(houses$bedrooms)) ** 2))
    )
  )
)
...
As we can see, the $\hat{SE}(\hat{\mu}(x_0-x_1))$ = `r se_x0_subtract_x1`. So our standard error for our difference is `r se_x0_subtract_x1`.
